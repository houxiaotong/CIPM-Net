...
import types
# from t_model import GModel
import matplotlib.pyplot as plt

parser = ArgumentParser(description='CIPM-Net-plus')

parser.add_argument('--epoch_num', type=int, default=5, help='epoch number of model')
...
parser.add_argument('--test_name', type=str, default='12', help='name of test set')
...
model.load_state_dict(torch.load('./%s/net_params_%d.pkl' % (model_dir, epoch_num)), strict=False)
...
with torch.no_grad():
    for img_no in range(ImgNum):
        ...
        cv2.imwrite("%s_CIPM_Net_plus_ratio_%d_epoch_%d_PSNR_%.2f_SSIM_%.4f.bmp" % (resultName, cs_ratio, epoch_num, rec_PSNR, rec_SSIM), im_rec_rgb)
        del x_output
...
output_file_name = "./%s/PSNR_SSIM_Results_MRI_CS_CIPM_Net_plus_layer_%d_group_%d_ratio_%d.txt" % (args.log_dir, layer_num, group_num, cs_ratio)
