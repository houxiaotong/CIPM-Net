...
class PatchEmbed(nn.Module):
    def __init__(self, img_size=256, patch_size=16, in_chans=1, embed_dim=16, norm_layer=None):
...
class PatchUnEmbed(nn.Module):
    def __init__(self, img_size=256, patch_size=16, in_chans=1, embed_dim=1, norm_layer=None):
        super().__init__()
        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
        patches_resolution = [img_size[0] // patch_size[0], img_size[1] // patch_size[1]]
...
if __name__ == '__main__':
    ...
    block = VSSBlock(hidden_dim=32,drop_path=0.1,attn_drop_rate=0.1,d_state=16,is_light_sr=False)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    block = block.to(device)

    B, H, W, C=1, 256, 256, 32
    input_tensor = torch.randn(B, H*W, C).to(device)
